@echo off
REM Script to create the NovaBackup Windows Service main.go file
set OUTPUT_FILE=D:\WORK_CODE\Backup\cmd\nova-service\main.go
del "%OUTPUT_FILE%" 2>nul

REM Create the file using PowerShell
powershell -Command "$code = @'" > "%TEMP%\write_service.ps1"

REM Write the Go code to the temp PowerShell script
type >> "%TEMP%\write_service.ps1" << 'GOCODE'
...
const (
	...
	serviceDescription = "Enterprise Backup & Disaster Recovery Platform - Provides API server and scheduled backup jobs"
	defaultPort        = 8080
)
...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, "NovaBackup service is starting...")

	if err := s.startComponents(); err != nil {
		s.logEvent(syscall.EVENTLOG_ERROR_TYPE, fmt.Sprintf("Failed to start components: %v", err))
		return true, 1
	}

	changes <- svc.Status{State: svc.Running, Accepts: cmdsAccepted}
	s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, fmt.Sprintf("NovaBackup service started successfully on port %d", defaultPort))
	...
	for {
		...
		case c := <-r:
			...
			case svc.Stop, svc.Shutdown:
				s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, "NovaBackup service is stopping...")
				changes <- svc.Status{State: svc.StopPending}
				if err := s.stopComponents(); err != nil {
					s.logEvent(syscall.EVENTLOG_ERROR_TYPE, fmt.Sprintf("Error during shutdown: %v", err))
				}
				s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, "NovaBackup service stopped successfully")
				break loop
			case svc.Pause:
				changes <- svc.Status{State: svc.Paused, Accepts: cmdsAccepted}
				s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, "NovaBackup service paused")
			case svc.Continue:
				changes <- svc.Status{State: svc.Running, Accepts: cmdsAccepted}
				s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, "NovaBackup service resumed")
			default:
				s.logEvent(syscall.EVENTLOG_WARNING_TYPE, fmt.Sprintf("Unexpected control request: %d", c))
			}
		}
	}
...
func (s *Service) startComponents() error {
	...
	s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, fmt.Sprintf("Opening database: %s", dbPath))
	s.db, err = database.NewSQLiteConnection(dbPath)
	if err != nil {
		return fmt.Errorf("failed to open database: %w", err)
	}

	s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, "Initializing scheduler...")
	...
	s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, "Scheduler started successfully")

	s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, fmt.Sprintf("Starting API server on port %d...", defaultPort))
	...
	go func() {
		if err := s.apiServer.Start(defaultPort); err != nil && err != http.ErrServerClosed {
			s.logEvent(syscall.EVENTLOG_ERROR_TYPE, fmt.Sprintf("API server error: %v", err))
		}
	}()
...
func (s *Service) stopComponents() error {
	...
	if s.apiServer != nil {
		...
		defer cancel()
		s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, "API server shutdown initiated")
	}
	...
	if s.scheduler != nil {
		if err := s.scheduler.Stop(); err != nil {
			...
			s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, "Scheduler stopped")
		}
	}
	...
	if s.db != nil {
		if err := s.db.Close(); err != nil {
			...
			s.logEvent(syscall.EVENTLOG_INFORMATION_TYPE, "Database connection closed")
		}
	}
...
func main() {
	...
	if isWindowsService {
		if err := runService(false); err != nil {
...
echo "'@" >> "%TEMP%\write_service.ps1"
echo [System.IO.File]::WriteAllText('%OUTPUT_FILE%', $code, [System.Text.UTF8Encoding]::new($false)) >> "%TEMP%\write_service.ps1"
echo Write-Host 'Created: %OUTPUT_FILE%' >> "%TEMP%\write_service.ps1"

REM Execute the PowerShell script
powershell -ExecutionPolicy Bypass -File "%TEMP%\write_service.ps1"

REM Clean up
del "%TEMP%\write_service.ps1"
