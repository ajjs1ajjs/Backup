...
export default function Backups() {
  ...
  const handleVerify = async (backupId) => {
    try {
      await fetchWithAuth(`/api/backups/${backupId}/verify`, { method: 'POST' });
      refetch();
    } catch (e) {
      refetch();
    }
  ...
  const handleDelete = async (backupId) => {
    try {
      await fetchWithAuth(`/api/backups/${backupId}`, { method: 'DELETE' });
      refetch();
    } catch (e) {
      refetch();
    }
          ...
          <Table>
            ...
            <TableBody>
              {backups.map((backup) => {
                ...
                return (
                  <TableRow key={backupId}>
                    <TableCell>{String(backupId || '').substring(0, 8)}...</TableCell>
                    ...
                    <TableCell>
                      <IconButton onClick={() => handleVerify(backup.backupId)} disabled={!backup.backupId}>
                        <VerifyIcon />
                      </IconButton>
                      <IconButton onClick={() => handleDelete(backup.backupId)} color="error" disabled={!backup.backupId}>
                        <DeleteIcon />
                      </IconButton>
                    </TableCell>
                  </TableRow>
