...
main() {
    ...
    log "  Username: admin"
    log "  Password: generated by the server on first start if not set in configuration"
    log ""
    log "IMPORTANT: Check the server logs for the bootstrap password and change it on first login."
    log ""
}
